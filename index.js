    ...
    try {
        ...
        switch (msgJson.msg) {
            ...
            case 'bucle':
                ...
                break;
            default:
                console.log('Mensaje recibido no coincide con ninguna acción esperada');
                break;
        }
...
async function employes() {
    //get("http://localhost:3333/syncEmployees", { timeout: 30000 })
    console.log("Employee sync sent...")
}

async function signings() {
    //get("http://localhost:3333/syncSignings", { timeout: 30000 })
    console.log("Signings sync sent...")
}

async function customers() {
    //get("http://localhost:3333/syncCustomers", { timeout: 30000 })
    console.log("Customers sync sent...")
}

async function items() {
    //get("http://localhost:3333/syncItems", { timeout: 30000 })
    console.log("Items sync sent...")
}

async function itemCategories() {
    //get("http://localhost:3333/syncItemCategories", { timeout: 30000 })
    console.log("ItemCategories sync sent...")
}

async function tickets() {
   //get("http://localhost:3333/syncSalesTickets", { timeout: 30000 })
